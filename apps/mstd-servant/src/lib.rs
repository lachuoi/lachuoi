...
async fn handle_cron_event(_: Metadata) -> anyhow::Result<()> {
    println!("Servant bot starting");

    let request = Request::builder()
        .method(Get)
        .uri("https://raw.githubusercontent.com/seungjin/lachuoi/refs/heads/main/assets/mstd-servant-news-feeds.json5")
        .build();
    let response: Response = spin_sdk::http::send(request).await?;
    let response_body = str::from_utf8(response.body()).unwrap();
    let rss_feeds: Value = serde_hjson::from_str(response_body).unwrap();

    if let Some(feeds) = rss_feeds.as_array() {
        for feed in feeds {
...
const DB_KEY_PREFIX: &str = "mstd-servant";
...
async fn rss_eater(name: String, url: String) -> anyhow::Result<()> {
    ...
    if check_process_lock(&name).await.unwrap().is_some() {
        println!("mstd-servant {} process lock exist - exit", name);
        println!("mstd-servant RSS finished");
        return Ok(());
    }
    ...
    .expect("mstd-servant Failed to parse date");
...
async fn post_to_mastodon(
    ...
    if msgs.is_empty() {
        println!("mstd-servant RSS - Nothing to publish");
        return Ok(());
    }
    ...
    for item in msgs {
        ...
        if response.status() == &200u16 {
            println!("mstd-servant-Rss published: {}", item.title.unwrap());
        }
    }
...
async fn check_process_lock(name: &String) -> anyhow::Result<Option<()>> {
    ...
    let naive_dt =
        NaiveDateTime::parse_from_str(updated_at, "%Y-%m-%d %H:%M:%S")
            .expect("mstd-servant Failed to parse datetime");
    ...
    if utc_dt < one_hour_ago {
        println!("mstd-servant lock process is older than 5 min. unlock it.");
...
async fn process_lock(name: &String) -> anyhow::Result<()> {
    println!("mstd-servant-rss {} process lock", name);
    let connection =
        Connection::open("lachuoi").expect("lachuoi db connection error");
